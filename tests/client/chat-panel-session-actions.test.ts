...
describe('ChatPanel session action menus', () => {
  ...
  it('routes all four current-session menu actions through the existing behavior', () => {
    ...
    // Fork feature: the local IndexedDB message cache needs an explicit tail
    // re-sync action, and it lives in this menu rather than a third header button.
    expect(source).toContain('void pullLatestMessages()')
    expect(source).toContain('pullLatest: t("chat.pullLatest")')
