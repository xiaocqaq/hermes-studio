...
export function buildActiveSessionMenuOptions(
  ...
  availability: ActiveSessionMenuAvailability = { canRename: true, canOpen: true, canPullLatest: true },
): DropdownOption[] {
  return [
    actionOption(labels.outline, 'outline', 'outline'),
    // Local fork feature: the chat store caches messages in IndexedDB, so an
    // explicit tail re-sync is the escape hatch when a cached view looks stale.
    actionOption(labels.pullLatest, 'pull-latest', 'refresh', { disabled: !availability.canPullLatest }),
